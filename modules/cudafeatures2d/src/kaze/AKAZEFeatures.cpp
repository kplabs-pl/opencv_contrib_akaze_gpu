...
void Matcher::bfmatch(cv::Mat &desc_query, cv::Mat &desc_train,
  ...
  MatchDescriptors(desc_query, desc_train, dmatches, pitch, descq_d, desct_d,
                   dmatches_d, dmatches_h);
  ...
  for (int i = 0; i < desc_query.rows; ++i) {
    ...
    // std::cout << dmatches_h[2*i].trainIdx << " - " <<
    // dmatches_h[2*i].queryIdx << std::endl;
...
cv::Mat Matcher::bfmatch_(cv::Mat desc_query, cv::Mat desc_train) {
  ...
  for (int i = 0; i < (int)dmatches_vec.size(); ++i) {
...
AKAZEFeatures::AKAZEFeatures(const AKAZEOptions &options) : options_(options) {
  ...
  if (options_.descriptor_size > 0 &&
      (options_.descriptor == cuda::AKAZE::DESCRIPTOR_MLDB_UPRIGHT ||
       options_.descriptor == cuda::AKAZE::DESCRIPTOR_MLDB)) {
...
/* ************************************************************************* */
void AKAZEFeatures::Allocate_Memory_Evolution() {
  float rfactor = 0.0;
...
int AKAZEFeatures::Create_Nonlinear_Scale_Space(const cv::Mat &img) {
  ...
  for (size_t i = 1; i < evolution_.size(); i++) {
    ...
    for (int j = 0; j < (int)nsteps_[i - 1]; j++) {
...
cv::Mat AKAZEFeatures::Feature_Detection_() {
...
void AKAZEFeatures::Feature_Detection(std::vector<cv::KeyPoint> &kpts) {
  ...
  for (size_t i = 0; i < evolution_.size(); i++) {
    ...
    float smax = 1.0f;
    if (options_.descriptor == cuda::AKAZE::DESCRIPTOR_SURF_UPRIGHT ||
        options_.descriptor == cuda::AKAZE::DESCRIPTOR_SURF ||
        options_.descriptor == cuda::AKAZE::DESCRIPTOR_MLDB_UPRIGHT ||
        options_.descriptor == cuda::AKAZE::DESCRIPTOR_MLDB)
      smax = 10.0 * sqrtf(2.0f);
    else if (options_.descriptor == cuda::AKAZE::DESCRIPTOR_MSURF_UPRIGHT ||
             options_.descriptor == cuda::AKAZE::DESCRIPTOR_MSURF)
...
/* ************************************************************************* */
void AKAZEFeatures::Save_Scale_Space() {
  cv::Mat img_aux;
  string outputFile;
  ...
  for (size_t i = 0; i < evolution_.size(); i++) {
    ...
    //    outputFile = "../output/evolution_" + to_formatted_string(i, 2) +
    //    ".jpg"; cv::imwrite(outputFile, img_aux);
  }
}

/* ************************************************************************* */
void AKAZEFeatures::Save_Detector_Responses() {
  cv::Mat img_aux;
  string outputFile;
  float ttime = 0.0;
  int nimgs = 0;
  ...
  for (size_t i = 0; i < evolution_.size(); i++) {
    ...
    if (ttime > 0) {
      ...
      //      outputFile =
      //          "../output/images/detector_" + to_formatted_string(nimgs, 2) +
      //          ".jpg";
