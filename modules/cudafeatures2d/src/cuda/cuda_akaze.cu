...
__global__ void Flow(float *imgd, float *flowd, int width, int pitch,
                     int height, DiffusivityType type, float invk) {
#define BW (SCHARR_W + 2)
  __shared__ float buffer[BW * (SCHARR_H + 2)];
  int tx = threadIdx.x;
  int ty = threadIdx.y;
  ...
  if (x < width && y < height && tx < SCHARR_W && ty < SCHARR_H) {
    ...
    if (type == cuda::AKAZE::DIFF_PM_G1)
      flowd[y * pitch + x] = exp(-dif2);
    else if (type == cuda::AKAZE::DIFF_PM_G2)
      flowd[y * pitch + x] = 1.0f / (1.0f + dif2);
    else if (type == cuda::AKAZE::DIFF_WEICKERT)
...
double Flow(CudaImage &img, CudaImage &flow, DiffusivityType type,
            float kcontrast) {
  // TimerGPU timer0(0);
  dim3 blocks(iDivUp(img.width, SCHARR_W), iDivUp(img.height, SCHARR_H));
  dim3 threads(SCHARR_W + 2, SCHARR_H + 2);
...
void FilterExtrema(cv::KeyPoint *pts, cv::KeyPoint *newpts, int* kptindices, int& nump) {
  ...
  for (int i=1; i<16; ++i) {
      maxnump = max<int>(maxnump,extremaidx_h[i]-extremaidx_h[i-1]);
  }
...
#pragma unroll
  ...
  //        int tBest = __shfl_down_sync(scoreBest,i);
  //        int tIdx = __shfl_down_sync(idxBest,i);
  //        if(tBest < scoreBest) {
  ...
  //            idxBest = tIdx;
  //        }
  //        tBest = __shfl_down_sync(scoreSecondBest,i);
  //        tIdx = __shfl_down_sync(idxSecondBest,i);
